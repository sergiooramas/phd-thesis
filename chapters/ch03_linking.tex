...
\part[Knowledge Extraction from Text]{Knowledge Extraction \\from Text}
...
For the creation of the ELMD dataset, the crowd-sourced annotations extracted from \texttt{Last.fm} biographies are combined with decisions made by \textsc{ELVIS} and its voting framework.\\%(see Figure~\ref{fig:linking:dataset_creator}).
...
Album & \parbox[t]{6cm}{DBpedia:Album, Yago:Album,\\ Schema:MusicAlbum} \\
Artist & \parbox[t]{6cm}{DBpedia:MusicalArtist, \\DBpedia:Band, Schema:MusicGroup, Yago:Musician, Yago:Creator, DBpedia:Artist} \\
Record Label & DBpedia:RecordLabel \\
\hline
	\end{tabular}
	\caption{Equivalence of types between Last.fm and DBpedia. Yago, Schema, and DBpedia refer to the correspondent ontologies.}
...
\begin{figure}[]
  \centering
	\includegraphics[width=\textwidth]{ch03_linking_pics/evaluation_both.png}
...
Here, the entity \textit{Ministry of Sound} refers to a Record Label, a spin-off of the well-known club, which is the entity that was incorrectly assigned: \url{dbpedia.org/resource/Ministry\_of\_Sound}. 
...
As shown in Figure~\ref{fig:linking:elmd}, different subsets of ELMD can be obtained at different confidence score thresholds, with Precision ranging from 0.94 to 0.97.
...
In addition, music information represented in DBpedia is not complete, as many existing artists, albums, and songs do not have a Wikipedia page. As a consequence, there are many annotated links in the biographies to \texttt{Last.fm} pages that does not have a corresponding DBpedia resource.
