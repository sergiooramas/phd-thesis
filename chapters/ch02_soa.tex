...
Open information extraction methods neither require an annotated corpus nor a pre-specified vocabulary, as they aim to discover all possible relations in the text \citep{Bankoetal2007}.
...
In \cite{Ziegler2004} taxonomic information is used to represents the user's interest in categories of products. %Consequently, user similarity is determined by common interests in categories and not by common interests in items. 
In \cite{Anand2007} the authors use an ontology of items to infer user preferences from rating data. %The system collaboratively generates recommendations using the ontology and infers preferences during similarity computation. 
...
Finally, another interesting direction about the usage of LOD is explored in \cite{MustoSLG14}, where the authors present a content-based context-aware recommendation framework that adopts a semantic representation based on distributional semantics and entity linking techniques.% In particular entity linking is used to detect entities in free text and map them to LOD.
...
%The remainder of this section is structured as follows: Sections \ref{sec:SOA:mir:text} and \ref{sec:SOA:mir:semantics} give a brief overview of research on text-based and knowledge-based representations, respectively, for MIR in general. Sections \ref{sec:SOA:mir:classfication} through \ref{sec:SOA:mir:music-recommendation} describe related work on the 3 MIR tasks that this thesis focuses on, namely music genre classification, artist similarity and music information retrieval
...
Music artist similarity has been studied from the score level, the acoustic level, and the cultural level \citep{Ellis2002}. In this thesis, we focus on the latter approach, and more specifically on text-based approaches. %Literature on document similarity, and more specifically on the application of text-based approaches for artist similarity is discussed next.
