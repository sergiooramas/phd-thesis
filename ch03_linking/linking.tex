...
In the remainder of this chapter, we first introduce \textsc{ELVIS} (Entity Linking Voting and Integration System), our EL integration and agreement approach. Then, we describe the text corpus we compiled from the \textsc{Last.fm} website and how it is combined with ELVIS. In the next step, the obtained dataset is evaluated. Then, a further process of automatic expansion of \textsc{ELMD} are describled. Finally, the manual annotation of a subset of the documents is presented. %Finally, we describe the resulting output of our system: The \textsc{ELMD} dataset. 
...
\begin{figure}[h!]
  \centering
	\includegraphics[height=3.25cm,width=8cm]{ch03_linking/pics/workflow.pdf}
...
\begin{figure}[!t]
  \centering
	\includegraphics[width=8cm]{ch03_linking/pics/evaluation_both.eps}
...
\begin{figure}[h!]
  \centering
	\includegraphics[width=8cm]{ch03_linking/pics/ELMD_Overview.eps}
...
%In addition, 100 more documents gathered also from Last.fm, but different to those in ELDM, were annotated to act as a test set. Thus, the ELMDGold dataset is composed of 200 documents, 100 for train and 100 for test. 
...
%In addition, by setting up a higher confidence threshold it is possible to obtain a subset of \textsc{ELMD} that prioritize higher Precision by sacrificing Recall (see Figure~\ref{fig:elmd}). 
