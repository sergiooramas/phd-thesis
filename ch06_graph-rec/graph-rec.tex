...
For this purpose we adopted Babelfy, a state of the art tool for Entity Linking and Word Sense Disambiguation \cite{Moro2014}. Babelfy maps words from a given text to entities in the BabelNet\footnote{\url{http://babelnet.org}} knowledge base. 
BabelNet is a multilingual encyclopedic dictionary that mixes knowledge from WordNet and Wikipedia. Thus, for every mapped and disambiguated text fragment, Babelfy returns the related WordNet \textit{synset}, and/or the related Wikipedia page (and its equivalent DBpedia resource).  %WordNet\footnote{\url{http://wordnet.princeton.edu}} is a large lexical database of English. In WordNet, words are grouped by synsets, which are sets of cognitive synonyms. DBpedia\footnote{\url{http://dbpedia.org}} instead is well-known encyclopedic knowledge base, built from Wikipedia, belonging to the Linked Open Data cloud. 
%In order to improve the disambiguation of text and tags associated to a sound, instead of processing textual descriptions and tags separately, they were concatenated and used together as input to Babelfy. 
%The idea was that both tags and description could benefit from each other in the disambiguation task as they enrich the representation of the corresponding context. 
%We also manually checked that the use of such strategy resulted in a better disambiguation than performing each linking separately. 
%Once the linking was performed, tags and description results were split. Specifically, for each entity detected in the description, the corresponding textual representation was linked to the sound node by using the \texttt{hasKeyword} property added to the original ontology.
% and to the entity by means of the isLinkedEntity property. 
%Mapped tags instead were linked to the sound by using the \texttt{hasTag} property. 
...
To show an example of entity linking performed by Babelfy we use the sound \texttt{prac-snare2.wav}\footnote{\url{http://www.freesound.org/people/TicTacShutUp/sounds/439/}} from Freesound. The description associated to this sound is \textit{"standard snare sample. lower/mid tuning on the head"} and tags are \textit{drums}, \textit{percussion}, \textit{snare}. Babelfy was able to detect and link most of the entities. Just to describe a few of them, the word \textit{sample} from the description was linked to the DBpedia entity \texttt{Sampling\_(music)}, the tag \textit{percussion} was mapped to the DBpedia entity \texttt{Rythm\_section}, the tag \textit{snare} was linked to the WordNet concept \texttt{snare\_drum.n.01} and DBpedia entity \texttt{Snare\_drum}. As shown in Figure~\ref{fig:graph_enhancement}, DBpedia entities and WordNet synsets are then further enriched with their related categories and hypernyms. Following the Linked Data principles\footnote{\url{http://www.w3.org/DesignIssues/LinkedData.html}}, we reused classes and properties from external vocabularies. The final knowledge graph after the entity linking and expansion process contains four main classes: \texttt{wordnet:Synset}, \texttt{Entity}, \texttt{Tag} and \texttt{skos:Concept} and seven relations: \texttt{hasTag}, \texttt{hasKeyword}, \texttt{wordnet:synset\_member}, \texttt{dcterms:relation}, \texttt{dcterms:subject}, \texttt{skos:broader} and \texttt{wordnet:hypernym}\footnote{All the prefix we use here are the ones available via the \url{http://prefix.cc} service}.
In particular, for the sounds recommendation dataset based on Freesound we further enriched the ontology originally developed in \cite{Font2014} as also shown in the left hand side of Figure~\ref{fig:graph_enhancement}. 
% (see Figure~\ref{fig:graph_enhancement}).
%We performed a manual check of many entity linking outputs and observed that sound textual descriptions, even when short, were very informative in most of the cases, and contained relevant keywords that were not explicitly represented in the list of tags. Figure \ref{fig:graph_enhancement} shows a portion of the final knowledge graph.

\begin{figure*}
\centering
\includegraphics[width=\textwidth]{ch06_graph-rec/pics/graph_all_final.pdf}
\caption{Portion of the final knowledge graph enriched with WordNet and DBpedia \label{fig:graph_enhancement}}
\end{figure*}


%{\color{red}
%\subsection{A Knowledge Graph for Sounds Description}\label{sec:kg4freesound}

%\subsection{Freesound}
%Freesound is an online collaborative sound database where people with diverse interest share recorded sounds under Creative Commons license. It started in 2005 as an initiative of the Music Technology Group of the University Pompeu Fabra. After nine years Freesound has become one of the most popular sites for sharing sound samples. It has now almost 4 million registered users and about 220,000 uploaded sounds. Among all registered users there is a low percent of highly engaged users that has been continuously contributing to the site, not only uploading sound, but also commenting, rating and discussing in the forum. A general characterization of its community of users can be found here \cite{Font2012}. 
%All sounds in Freesound have attached a number of tags and a textual description, which are added by contributors at the time of uploading. Only registered users can download the sounds published by other users, but anyone who access the site can search, browse and listen to sounds. 
%Registered users may also comment and rate sounds, giving a score between 1 and 5.

%Freesound allows the users to share sounds on the Web. They are all described through a number of tags and a textual information, which are added by contributors at the time of their upload. Only registered users can download the sounds published by other users, but anyone who access the site can search, browse and listen to sound previews. User generated content in Freesound is described by means of a collaborative tagging system, where users assign free-text keywords to content resources. Thus, a folksonomy emerges as the resulting set of associations between tags, users and resources. 

%A description regarding the folksonomy of Freesound has been presented in \cite{Font2012a}. 
%Such folksonomy is currently used in a tag recommendation system \cite{Font2014b} whose task is assisting users in tagging sounds at the time of upload. %The description of the tag recommender can be found in \cite{Font2014b}. 
%The main aim in tag recommendation systems is to provide to the user with a list of tags she can use to annotate a resource.
%Although this tripartite model has been widely used for recommender systems, 
%As pointed out by \cite{Gemmell2012} in social tagging systems the problem of resource recommendation has received less attention than the problem of tag recommendation. 
%In this work we focus on the problem of sound recommendations which consists in suggesting to the user a list of sounds she might be interested in. 

%Based on the Freesound folksonomy, a specific tagging ontology was created and used to initially classify the tags most frequently associated to sounds \cite{Font2014}. 

%We now describe the Freesound tagging ontology, and then we show how we built our knowledge graph related to sounds by enriching the data classified via the ontology with external knowledge sources by means of entity linking tools. 
%\subsection{Freesound Ontology}
%Despite its successful use, folksonomies suffer from various drawbacks, e.g. ambiguity, informational redundancy, idiosyncratic naming conventions \cite{Halpin2006}. In order to cope with these issues, many approaches have been proposed aimed to incorporate semantic-enabled knowledge, such as ontologies, into folksonomies. %The combination of folksonomies and ontologies can be addressed in different ways \cite{Oramas2014}. A tagging ontology may act as an upper structure that defines the tagging process, and be used as a common schema to interconnect different folksonomies. Another approach is to manually or automatically derive a domain ontology from the implicit knowledge present in the folksonomy. 

%The Freesound Ontology is defined by extending an upper tagging ontology with domain specific knowledge. 

%Starting from this definition, an ontology that represents the tagging process in Freesound was defined. 

%The ontology has five classes (see the yellow boxes in Figure \ref{fig:fso_enhancement}): \texttt{Sound}, \texttt{Tag}, \texttt{User}, \texttt{TagSet} and \texttt{SoundSet}. Sounds may be \texttt{uploaded}, \texttt{downloaded} and \texttt{rated} by a \texttt{User}, and may have one or more associated tags represented through the property \texttt{hasTag}. Domain specific knowledge is represented by the classes \texttt{TagSet} and \texttt{SoundSet}. Tag sets represent different types of tags according to semantic characteristics (e.g. Instrument, Location, etc.). Sound sets represent categories of sounds (they are related via the \texttt{classifiedAs} property) according to acoustic characteristics (e.g. InstrumentSample, Voice). Tags classified into a tag set (via \texttt{inSet}) may be relevant for a specific sound set, and this is represented by the relation \texttt{usedBy} between \texttt{TagSet} and \texttt{SoundSet} classes. %This ontology schema is described in Figure \ref{fig:fso_enhancement}.
%Initially, 26 sets of tags, 5 sets of sounds, and 24 relations of applicability between tag and sound sets were defined and 432 frequently used tags were manually classified into tag sets. Although these classified tags represent 1\% of the total number of tags in Freesound, they appear in more than 85\% of sound annotations. 

%The available data has been thus represented as an RDF graph instantiating the Freesound ontology. This represents the basis of our knowledge graph (see the left hand side of Fig. \ref{fig:graph_enhancement}).

%In order to add more semantics to the description of sounds, we exploit contextual information, i.e., tags and text descriptions, to link more entities to the ones already available in the Freesound Ontology. To this end, we applied the semantic enrichment methodology described above. 
%To represent the new entities we extended the original ontology by plugging in a new set of classes and related properties (see the green dashed arrows and the green boxes in Fig. \ref{fig:fso_enhancement}). Following the Linked Data principles\footnote{\url{http://www.w3.org/DesignIssues/LinkedData.html}}, we reused classes and properties from external vocabularies. The final ontology after the entity linking and expansion process contains three new classes: \texttt{wordnet:Synset}, \texttt{Entity} and \texttt{skos:Concept} and five new relations: \texttt{wordnet:synset\_member}, \texttt{dcterms:relation}, \texttt{dcterms:subject}, \texttt{skos:broader} and \texttt{wordnet:hypernym}\footnote{All the prefix we use here are the ones available via the \url{http://prefix.cc} service}.


%\begin{figure}
%\centering
%\includegraphics[width=0.4\textwidth]{ch06_graph-rec/pics/fso_graph.png}
%\caption{Portion of the initial knowledge graph\label{fig:fso_graph}}
%\end{figure}

%Following the Linked Open Data principles, the classes \texttt{Synset} and \texttt{Category} have been defined as \url{owl:equivalentClass} respectively with \url{wordnet:Synset} (see \url{http://wordnet-rdf.princeton.edu/ontology}) and \url{skos:Concept} (see \url{http://www.w3.org/TR/skos-reference/}) while the five new properties have been defined as \url{owl:equivalentProperty} respectively with \url{wordnet:synset_member}, \url{dcterms:relation} (see \url{http://dublincore.org/documents/dcmi-terms/}), \url{dcterms:subject}, \url{skos:broader} and  \url{wordnet:hypernym}.
%, and fulfilled with the information gathered from WordNet and DBpedia.  
%The knowledge graph was then built following the same procedure described for the initial Freesound Ontology, but according to the schema of the new enriched ontology. 

%\begin{figure}
%	\centering
%	\includegraphics[width=0.70\textwidth]{ch06_graph-rec/pics/fso_enhancement_new2.png}
%	\caption{The enriched Freesound Ontology. \label{fig:fso_enhancement}}
%\end{figure}
%}

%Moreover, many meaningful entities were mapped to these words
%This was a correct mapping, and the word \textit{sample} defines perfectly the kind of sound, so it would be a very adequate tag. 
%After the analysis of the Entity Linking and Word Sense Disambiguation output, it was observed that sound textual descriptions were very informative in most of the cases, and contain relevant keywords that were not in the list of tags. Moreover, many meaningful entities were mapped to these words. For example, in a sound\footnote{\url{http://www.freesound.org/people/TicTacShutUp/sounds/439/}}, the description was \textit{"standard snare sample. lower/mid tuning on the head"}" and tags were \textit{drums}, \textit{percussion}, \textit{snare}. Babelfy linked the word \textit{sample} from the description with the DBpedia entity \textit{Sampling\_(music)}. This was a correct mapping, and the word \textit{sample} defines perfectly the kind of sound, so it would be a very adequate tag. 
%Therefore, an expansion of the semantic network was performed using words from the sound context. For this purpose, every word from the textual description of a sound that was not a stop word was considered as a keyword, and was related to the sound through the property hasKeyword. 
%Two kinds of tags were then stored, the original tags and the extracted keywords. In addition, if Babelfy identified a set of consecutive words as an entity, the concatenation of these words was treated as a single keyword. 
%Thus, after these processes of semantic and contextual expansion, the number of triples in our knowledge base was considerably increased.
...
Due to the high volume of users we deal with in our Freesound dataset (see Section \ref{sec:evaluation}), we focused on learning methods which are computationally efficient. For this reason we adopted the approach of computing the explicit feature mapping of the item graphs and use linear methods to learn the user model. Specifically, we use the Linear Support Vector Regression \cite{HoL12} algorithm. 
...
%Since we are interested in the individual items by themselves we focus on each \texttt{item neighborhood graph}, that is the sub-graph describing that item.  
...
Figure \ref{fig:Example} shows an example of 3-hop item neighborhood graph for item $i$, namely $G^3_i$. We see that, if we consider the shortest path, all the entities are no more than 3 hops distant from $i$. 
\begin{figure}
\begin{center}
...
\label{fig:Example}
\end{figure}
To clarify the definition and computation of $G^h_i$ and $E^h_i$ for item $i$, we show their computation with reference to the example shown in Figure \ref{fig:Example}:\\ 
...
With reference to example showed in Figure \ref{fig:Example}, the $c_{l,e_m}$ values are computed as follows: 
...
The presented graph embedding is an adaptation of the one presented in \cite{ODMD14a}, in this work we use a logarithmic discount factor instead of a parametric one.  
%In this mapping each feature refers to an entity in $E$ and the corresponding score represents the weight associated to that entity in $G^h_i$. The resulting feature vector $\phi_E(G^h_i)$ is:
%%\begin{small}
%\[
%\phi_E(G^h_i)=(w_{i,e_1} ,w_{i,e_2} ,...w_{i,e_m} ,...,w_{i,e_t} )
...
\begin{table}
	%[!htb]
	%\tbl{Datasets Overview\label{tbl:datasets}}{%
%	\scriptsize
	\label{tbl:datasets}
...
This collaborative information was complemented with the list of top tags of every song provided by the Last.fm API, and a textual description of each song coming from Songfacts.com. Information about the enriched knowledge graph is shown in Table~\ref{tbl:datasets}.
...
The implementation of the recommendation algorithm presented in this work is available on GitHub \footnote{\url{https://github.com/sisinflab/lodreclib}}.
...
\begin{table}
	%[!htb]
	%\tbl{Freesound Results\label{tbl:Res1}}{%
%	\scriptsize
	\label{tbl:Res1}
\begin{tabular}{l l c c c c c c }
	...
	\caption{Accuracy, Novelty and Aggregate Diversity results for different versions of the Freesound dataset. Best values in each column are in bold. The * symbol indicates best values for hybrid and collaborative configurations. \texttt{Ent} and \texttt{Path} refers to graph embedding options; \texttt{fso}, \texttt{wn} and \texttt{db} to the initial Freesound Ontology, WordNet and DBpedia respectively; \texttt{tags} to item tags, and \texttt{keyw} to text description keywords; \texttt{h} indicates the length of the h-hop neighborhood graph; \texttt{Collab} means that only collaborative features are considered; \texttt{noCollab} that no collaborative features are considered; \texttt{VSM} refers to Vector Space Model embedding; \texttt{Audio Sim}  to the audio-based approach.
	}
\end{table}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%In \texttt{(fso+wn+db/h=x)} the item is represented by the corresponding \texttt{x}-hop neighborhood extracted from the FSO ontology enriched with WordNet and DBpedia. 
%Differently from \texttt{(fso+wn+db/h=x)}, in the configuration \texttt{(fso+wn+db+keyw+tag/h=x)} also text descriptions are mapped to the ontology. 
%Differently from the previous cases, \texttt{VSM keyw+tags} is a pure textual approach where the Vector Space Model was applied on raw tags and keywords. 
%\texttt{noCollab} means that no collaborative features are considered (pure content-based approach) and \texttt{Collab} means that only collaborative features are considered (each feature vector consists of $\phi_{col}$ only). 
In addition, we built a content-based approach purely based on 352 low-level audio features\footnote{\url{https://www.freesound.org/docs/api/analysis_example.html\#all-descriptors}} extracted from the sound signal by using Essentia~\cite{Bogdanov2013}. In this approach, predictions are computed by aggregating the Euclidean distances between the sounds downloaded by the user and the target sound to recommend. %Final recommendation lists are generated by taking the sounds with lowest score.
All the results are reported in Table \ref{tbl:Res1}. 
...
\begin{figure*}
	...
	\end{subfigure}
	\caption{Precision-Recall, Novelty and Aggregate Diversity plots in Freesound dataset\label{fig:accur}}
\end{figure*}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\begin{figure*}
%	\centering
%	\subfigure{\includegraphics[width=1\textwidth]{ch06_graph-rec/pics/prec_rec.png}}
%	\subfigure{\includegraphics[width=1\textwidth]{ch06_graph-rec/pics/nov.png}}
%	\subfigure{\includegraphics[width=1\textwidth]{ch06_graph-rec/pics/adiv.png}}
%	\caption{Precision-Recall, Novelty and Aggregate Diversity plots\label{fig:accur}}
%\end{figure*}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%In order to evaluate the effectiveness in terms of ranking accuracy of \framework we compared it with several state of the art recommendation algorithms:
...
\texttt{BPR Linear} is a hybrid matrix factorization method able to work with sparse datasets \cite{GantnerDFRS10}. We used keywords and tags as item attribute data. 
...
%To confirm the obtained results and its applicability to other musical contexts, we performed a second experiment on the Last.fm dataset using the same set of feature vector variants and competitor approaches. %Results on the evaluation of the semantic item description enhancement are shown in Table~\ref{tbl:Res_sf}.
%, whilst the comparison with other methods is shown in Figure~\ref{fig:accur_sf}. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{table}
	%[!htb]
	%\tbl{Last.fm Results}{%
...
%		Ent(wn+db/tags/h=2) & \textbf{0.612} & 0.321 & \textbf{0.122} & 2.465 & 0.342 \\
...
%		VSM tags - noCollab & 0.127 & 0.048 & 1.050 & \textbf{0.757} \\	
		\bottomrule		
	\end{tabular}
...
\paragraph*{\textbf{Evaluation of the semantic item description enhancement}}\label{sem_eval}
As we may notice from the results shown in Table~\ref{tbl:Res_sf}, Entity-based embedding, \texttt{Collab}, and \texttt{VSM tags} approaches have very similar performance in terms of precision and recall. The first two Entity-based embedding variants have slightly higher MRR than \texttt{VSM tags}, meaning that they better locate relevant items in the top positions. Analogously to the previous sounds recommendation task, the approaches exploiting semantic expansion outperform the others in terms of novelty and aggregated diversity. The same tendency of the previous experiment is observed with the Entity-based and Path-based item neighborhood mappings. The Path-based approaches have lower precision, but much better novelty and aggregated diversity. Moreover, it is very interesting to observe that for both embedding options if we expand the graph by means of farther entities (h=4) precision decreases whilst novelty and diversity improve. It is noteworthy thet differently from the results of the Freesound experiment, here we obtain higher accuracy with the approach that uses only tags and not keywords. Our interpretation of this trend is that, as shown in Table~\ref{tbl:datasets}, the number of tags in the Freesound dataset is somehow scarce, and the addition of keywords taken from the textual descriptions improves the annotation of the items. On the other side, in the Last.fm dataset, the set of tags is already very rich, then the addition of keywords introduces noise within the items description thus deteriorating the accuracy of recommendations. 
...
\begin{figure*}
	...
	\end{subfigure}
	\caption{Precision-Recall, Novelty and Aggregate Diversity plots in Last.fm dataset\label{fig:accur_sf}}
\end{figure*}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
